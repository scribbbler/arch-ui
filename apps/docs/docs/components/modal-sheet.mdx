...
title: "Modal sheet"
description: "A mobile-first overlay that combined modal dialog behaviour with a bottom sheet presentation."
...
<div style={{background: '#F3F3F3', borderRadius: '12px', padding: '32px', margin: '24px 0', display: 'flex', justifyContent: 'center'}}>
  <div style={{maxWidth: '400px', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '200px'}}>
    <Button kind="secondary" onClick={() => alert('Modal sheet would open here')}>Open Modal Sheet</Button>
  </div>
</div>
