...
title: "Button group"
description: "A group of buttons that function as a single control for toggling between options."
eyebrow: "BUTTONS"
status: "Draft"
hide_title: true
---
...
import { ButtonGroup } from '../../../../packages/components/src/ButtonGroup';
...
<div style={{background: '#F3F3F3', borderRadius: '12px', padding: '32px', margin: '24px 0', display: 'flex', justifyContent: 'center'}}>
  <ButtonGroup selected={0} onChange={() => {}}>
    <Button>One</Button>
    <Button>Two</Button>
    <Button>Three</Button>
  </ButtonGroup>
...
A button group wraps multiple button segments inside a single container. Both the container and at least two buttons are required.
...
<div style={{background: '#f3f3f3', borderRadius: '12px', padding: '48px 32px', margin: '24px 0', display: 'flex', justifyContent: 'center'}}>
  <div style={{position: 'relative', display: 'inline-block'}}>
    ...
    <div style={{position: 'absolute', bottom: '-28px', left: '50%', transform: 'translateX(-50%)', fontSize: '13px', color: '#DE1135', fontWeight: 700, textAlign: 'center', whiteSpace: 'nowrap'}}>Buttons<br/><span style={{fontWeight: 400}}>(Required)</span></div>
    <div style={{position: 'absolute', top: '50%', right: '-40px', transform: 'translateY(-50%)', fontSize: '11px', color: '#DE1135', fontWeight: 600}}>4px</div>
  </div>
</div>
