...
sidebar_label: Toast
title: "Toast"
description: "A brief, non-blocking notification that appears in a corner of the screen to inform the user of an event or status change."
eyebrow: "MESSAGING"
status: "Draft"
hide_title: true
---
...
<div style={{background: '#F3F3F3', borderRadius: '12px', padding: '32px', margin: '24px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px'}}>
  <p style={{fontSize: '14px', color: '#5E5E5E', margin: 0}}>Toasts are triggered imperatively via the <code>useToast</code> hook. Wrap your app in <code>ToastProvider</code> to enable them.</p>
  <Button kind="secondary" size="compact" onClick={() => alert('In a live app, this would trigger: toast({ title: "Saved", variant: "success" })')}>Show Toast</Button>
</div>
