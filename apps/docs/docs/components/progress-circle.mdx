...
description: "A circular progress indicator that communicates completion percentage in a compact, radial format."
...
export const CircleTrack = ({ value = 0, size = 72, strokeWidth = 6 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{display: 'block'}}>
      ...
      {value > 0 && (
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#266EF1" strokeWidth={strokeWidth}
